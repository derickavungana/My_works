...
def delete_branch(page, branch_name):
    ...
    branch_locator = page.locator(f"text={branch_name}")
    if branch_locator.count() > 0 and branch_locator.first.is_visible():
        branch_locator.first.dblclick()
        page.wait_for_timeout(1000)
    else:
...
def main():
    ...
    with sync_playwright() as play:
        ...
        try:
            branch_details = generate_random_bank_branch_details()
            bank_name = branch_details['bank_name']
            branch_name = branch_details['branch_name']
            branch_code = branch_details['branch_code']
            branch_notes = branch_details['branch_notes']

            print(f"\n{'=' * 60}")
            print("Generated Bank Branch Details:")
            print(f"  Bank Name: {bank_name}")
            print(f"  Branch Name: {branch_name}")
            print(f"  Branch Code: {branch_code}")
            print(f"  Branch Notes: {branch_notes}")
            print(f"{'=' * 60}\n")
            ...
            print(f"➤ Creating branch '{branch_name}' for bank '{bank_name}'...")
            create_bank_branch(page, bank_name, branch_name, branch_code, branch_notes)
            ...
            branch_found = search_branch(page, branch_name)
            if branch_found:
                print(f"✓ Branch '{branch_name}' found successfully\n")
            else:
                print(f"✗ Branch '{branch_name}' not found\n")
                return

            print(f"➤ Activating branch '{branch_name}'...")
            status = toggle_branch_active_status(page, branch_name)
            if status == "true":
                print(f"✓ Branch '{branch_name}' is now ACTIVE (Checkbox checked)\n")
            elif status == "false":
                print(f"✓ Branch '{branch_name}' is now INACTIVE (Checkbox unchecked)\n")
            else:
                print(f"? Branch '{branch_name}' status: {status}\n")

            print(f"➤ Setting Approved status for branch '{branch_name}'...")
            approved_status = click_checkbox_by_label(page, "Approved:")
            if approved_status == "true":
                print(f"✓ Branch '{branch_name}' is now APPROVED (Checkbox checked)\n")
            elif approved_status == "false":
                print(f"✓ Branch '{branch_name}' is now NOT APPROVED (Checkbox unchecked)\n")
            else:
                print(f"? Branch '{branch_name}' approved status: {approved_status}\n")
            ...
            print(f"➤ Searching for branch '{branch_name}' to delete...")
            deletion_started = delete_branch(page, branch_name)
            if deletion_started:
                print(f"✓ Delete button clicked, confirming deletion...\n")
                if confirm_deletion(page):
                    print(f"✓ Branch '{branch_name}' deleted successfully\n")
                    print(f"➤ Verifying branch deletion...\n")
                    verify_branch_deleted(page, branch_name)
                else:
